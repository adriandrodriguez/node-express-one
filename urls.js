...
function urlInfo (urls) {
  urls.forEach(async url => {
    ...
    const thirdSlashIndex =
      url.indexOf('/', afterSecondSlashIndex) !== -1
        ? url.indexOf('/', afterSecondSlashIndex)
        : null
    const fileName =
      url.slice(afterSecondSlashIndex, thirdSlashIndex || url.length) + '.txt'
    try {
